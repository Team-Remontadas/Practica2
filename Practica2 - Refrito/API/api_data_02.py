...
def create_certification(token):
    if not request.json or \
            not 'identifiacion' in request.json or \
            not 'nombre' in request.json or \
            not 'celular' in request.json or \
            not 'correo' in request.json:
        abort(400)

    data = {
        'identifiacion': request.json['identifiacion'],
        ...
        if datos == None:
            ...
            for collect in datos:
                lista.append({"id": str(collect['_id']),
                      "identifiacion": collect['identifiacion'],
                      "nombre": collect['nombre'],
                      "celular": collect['celular'],
                      "correo": collect['correo'],
                      "token":collect['token']})
        ...
        if datos == None:
            ...
            data = {
                ...
                "data": {"id": str(datos['_id']),
                        "identifiacion": datos['identifiacion'],
                        "nombre": datos['nombre'],
...
def create_job():
    ...
    try:
        ...
        abort(500)
    return jsonify({'jobs': salida}), 201
from flask import request

@app.route('/jobs', methods=['GET'])
def getting_job_info():
    try:
        empresa = request.args.get('empresa')
        if empresa:
            datos = conexDB.jobs.find({'enterprise': empresa})
        else:
